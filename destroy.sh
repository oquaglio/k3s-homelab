...
echo -e "${YELLOW}Step 3: Deleting Apache Airflow (Helm)...${NC}"
helm uninstall airflow --namespace airflow 2>/dev/null || true
# Delete PVCs
kubectl delete pvc -n airflow --all --ignore-not-found=true
# Delete manual migration pod
kubectl delete pod airflow-db-init -n airflow --ignore-not-found=true
echo -e "${GREEN}✓ Apache Airflow deleted${NC}"
echo ""
...
echo -e "${YELLOW}Step 8: Deleting Portainer...${NC}"
...
echo -e "${YELLOW}Step 11: Deleting namespaces...${NC}"
